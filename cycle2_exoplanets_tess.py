...
final_jwstdf.to_csv(ddir + "jwst_cycle2_planet_prop_tess.csv")
...
final_uniqueprev.to_csv(ddir + "transitspec_planet_prop_tess2.csv")


#%%
#Read in my tables, which were created above
#Adjust the parameters for one that is known to be in correct in exo.mast
final_uniqueprev = pd.read_csv(ddir+"transitspec_planet_prop_tess2.csv")
final_jwstdf = pd.read_csv(ddir + "jwst_cycle2_planet_prop_tess.csv")
...
plt.plot(go1_transit['Tp'], go1_transit['Rp']*Mj, 's', color="darkorange",
             fillstyle='full', label="JWST GO1 Transit", ms=8, alpha=0.9)
plt.plot(go2_transit['Tp'], go2_transit['Rp']*Mj, '*', color="yellow",
...
#plt.savefig(ddir+"cycle2_planets.png")
...
#%%
#Teq vs Radius and stellar temperature 
...
if mark_tess:
    plt.savefig(ddir+"cycle2_planets_tess_star.png")
else:
    plt.savefig(ddir+"cycle2_planets_star.png")
...
cy2_transit = transit[(jwstdf['calltype'] == 'Cy2' )]
...
if mark_cy2:
    plt.plot(cy2_transit['inFlux'], cy2_transit['Rp']*Mj, 'D', color="white", 
             fillstyle='none', ms=12, label="Cycle 2")
...
if mark_cy2:
    plt.savefig(ddir+"cycle12_planets_cy2_influx.png")
else:
    plt.savefig(ddir+"cycle12_planets_influx.png")
#%%
...
plt.savefig(ddir+"cycle2_planets_influx.png")
